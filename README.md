...
| 语音识别 `/asr/submit` | `audio` 文件或 `audio_url`（二选一） | multipart/form-data；音频不超过 60 分钟、100MB；支持 wav/mp3/ogg/flac/m4a/aac/opus/webm/raw；可选 language/rate/bits/channel/hotwords | `audio_url` 用例统一使用 `common.asr_audio_url`；本地文件用例使用 `valid_audio`、`valid_speaker_5min_50mb`、`valid_audio_65min`、`valid_audio_205MB`、`invalid_audio` |
